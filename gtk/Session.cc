...
private:
    ...
    sigc::signal<void(Torrent::ChangeFlags)> signal_torrents_changed_;
    ...
    Glib::RefPtr<TorrentSorter> sorter_;
    ...
    , session_(session)
{
    ...
    sorter_ = TorrentSorter::create();
    signal_torrents_changed_.connect(sigc::mem_fun(*sorter_, &TorrentSorter::update));
...
int compare_torrents_by_id(Glib::RefPtr<Torrent const> const& lhs, Glib::RefPtr<Torrent const> const& rhs)
{
    auto const lhs_id = lhs->get_id();
    auto const rhs_id = rhs->get_id();
    return lhs_id < rhs_id ? -1 : (lhs_id > rhs_id ? 1 : 0);
}

std::pair<Glib::RefPtr<Torrent>, guint> find_row_from_torrent_id(
    Glib::RefPtr<Gio::ListStore<Torrent>> const& model,
    tr_torrent_id_t torrent_id)
{
    for (auto i = 0U, count = model->get_n_items(); i < count; ++i)
    {
        if (auto const item = model->get_item(i); item->get_id() == torrent_id)
        {
            return { item, i };
        }
    }
...
void Session::Impl::on_torrent_metadata_changed(tr_torrent* tor)
{
    Glib::signal_idle().connect(
        [this, core = get_core_ptr(), torrent_id = tr_torrentId(tor)]()
        {
            ...
            if (auto const [item, position] = find_row_from_torrent_id(raw_model_, torrent_id); item != nullptr)
            {
                item->update();
            }
...
void Session::add_torrent(tr_torrent* torrent, bool do_notify)
{
    impl_->add_torrent(torrent, do_notify);
}

void Session::Impl::add_torrent(tr_torrent* torrent, bool do_notify)
{
    if (torrent != nullptr)
    {
        raw_model_->insert_sorted(Torrent::create(*torrent), &compare_torrents_by_id);

        if (do_notify)
        {
            gtr_notify_torrent_added(get_core_ptr(), tr_torrentId(torrent));
        }

        signal_torrents_changed_.emit(Torrent::ChangeFlags().set());
    }
}
...
void Session::torrent_changed(tr_torrent_id_t id)
{
    ...
    if (auto const [item, position] = find_row_from_torrent_id(model, id); item != nullptr)
    {
        item->update();
    }
}
...
void Session::remove_torrent(tr_torrent_id_t id, bool delete_files)
{
    ...
    if (tor != nullptr)
    {
        ...
        // NOTE: Linear search, maybe implement Gio::ListModel ourselves
        if (auto const [item, position] = find_row_from_torrent_id(impl_->get_raw_model(), id); item != nullptr)
        {
        ...
        impl_->signal_torrents_changed().emit(Torrent::ChangeFlags().set());
    }
}
...
void Session::load(bool force_paused)
{
    ...
    {
        impl_->get_raw_model()->insert_sorted(Torrent::create(*torrent), &compare_torrents_by_id);
    }
...
{
    auto flags = Torrent::ChangeFlags();

    /* update the model */
    for (auto i = 0U, count = raw_model_->get_n_items(); i < count; ++i)
    {
        flags |= raw_model_->get_item(i)->update();
    }
    ...
    if (flags.any())
    {
        signal_torrents_changed_.emit(flags);
    }
}
...
sigc::signal<void(Torrent::ChangeFlags)>& Session::signal_torrents_changed()
{
