...
void update_cache_value(T& value, U&& new_value, std::bitset<Torrent::CHANGE_FLAG_COUNT>::reference change_flag)
{
    if (value != new_value)
    {
...
template<typename T, typename U, typename = std::enable_if_t<std::is_floating_point_v<T>>>
void update_cache_value(T& value, U new_value, T epsilon, std::bitset<Torrent::CHANGE_FLAG_COUNT>::reference change_flag)
{
...
public:
    ...
    void notify_property_changes(ChangeFlags flags) const;
...
private:
    Glib::ustring get_short_transfer() const;
    Glib::ustring get_error() const;
    Glib::ustring get_activity() const;
...
Torrent::ChangeFlags Torrent::Impl::update_cache()
{
    ...
    update_cache_value(
        ...
        result[ChangeFlag::ACTIVE_PEERS_DOWN]);
    ...
    update_cache_value(
        ...
        std::clamp(
            ...
            1.0F),
        0.01F,
        result[ChangeFlag::PERCENT_DONE]);
    ...
    update_cache_value(
        ...
        result[ChangeFlag::ACTIVE_PEER_COUNT]);
    ...
    update_cache_value(
        ...
        0.01F,
        result[ChangeFlag::SEED_RATIO_PERCENT_DONE]);
...
void Torrent::Impl::notify_property_changes([[maybe_unused]] ChangeFlags flags) const
{
#if GTKMM_CHECK_VERSION(4, 0, 0)

    static auto const short_status_flags = make_change_flags({
        ChangeFlag::ACTIVE_PEERS_DOWN,
        ChangeFlag::ACTIVE_PEERS_UP,
        ChangeFlag::ACTIVITY,
        ChangeFlag::FINISHED,
        ChangeFlag::RATIO,
        ChangeFlag::RECHECK_PROGRESS,
        ChangeFlag::SPEED_DOWN,
        ChangeFlag::SPEED_UP,
    });
    static auto const long_progress_flags = make_change_flags({
        ChangeFlag::ACTIVITY,
        ChangeFlag::ETA,
        ...
        ChangeFlag::RATIO,
        ChangeFlag::TOTAL_SIZE,
    });
    static auto const long_status_flags = make_change_flags({
        ChangeFlag::ACTIVE_PEERS_DOWN,
        ChangeFlag::ACTIVE_PEERS_UP,
        ChangeFlag::ACTIVITY,
        ChangeFlag::ERROR_CODE,
        ChangeFlag::ERROR_MESSAGE,
    ...
    static auto const css_classes_flags = make_change_flags({
        ChangeFlag::ACTIVITY,
        ChangeFlag::ERROR_CODE,
    });

    if (flags.test(ChangeFlag::MIME_TYPE))
    {
        property_icon_.notify_changed(torrent_);
    }

    if (flags.test(ChangeFlag::NAME))
    {
        property_name_.notify_changed(torrent_);
    }

    if (flags.test(ChangeFlag::PERCENT_DONE))
    {
        property_percent_done_.notify_changed(torrent_);
    }

    if ((flags & short_status_flags).any())
    {
        property_short_status_.notify_changed(torrent_);
    }

    if ((flags & long_progress_flags).any())
    {
        property_long_progress_.notify_changed(torrent_);
    }

    if ((flags & long_status_flags).any())
    {
        property_long_status_.notify_changed(torrent_);
    }

    if (flags.test(ChangeFlag::ACTIVITY))
    {
        property_sensitive_.notify_changed(torrent_);
    }

    if ((flags & css_classes_flags).any())
    {
    ...
    g_signal_emit_by_name(torrent_.gobj(), "notify", nullptr);
...
Glib::ustring Torrent::Impl::get_short_status() const
{
...
Glib::ustring Torrent::Impl::get_long_progress() const
{
    Glib::ustring gstr;
...
Glib::ustring Torrent::Impl::get_long_status() const
{
    auto status_str = get_error();
    if (status_str.empty())
    {
        status_str = get_activity();
    }
    ...
    default:
        if (auto const buf = get_short_transfer(); !std::empty(buf))
        {
...
Glib::ustring Torrent::Impl::get_short_transfer() const
{
...
Glib::ustring Torrent::Impl::get_error() const
{
...
Glib::ustring Torrent::Impl::get_activity() const
{
    switch (cache_.activity)
    {
    case TR_STATUS_STOPPED:
...
tr_torrent& Torrent::get_torrent() const
{
...
Glib::ustring Torrent::get_short_status() const
{
    return impl_->get_short_status();
}

Glib::ustring Torrent::get_long_progress() const
{
    return impl_->get_long_progress();
}

Glib::ustring Torrent::get_long_status() const
{
    return impl_->get_long_status();
}
