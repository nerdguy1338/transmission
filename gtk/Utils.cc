...
void convert_widget_to_bin_window_coords(Gtk::TreeView const& view, int view_x, int view_y, int& event_x, int& event_y)
{
    view.convert_widget_to_bin_window_coords(view_x, view_y, event_x, event_y);
}

void convert_widget_to_bin_window_coords(Gtk::ListView const& /*view*/, int view_x, int view_y, int& event_x, int& event_y)
{
    event_x = view_x;
    event_y = view_y;
}
    ...
    if (press_callback)
    {
        controller->signal_pressed().connect(
            [&view, press_callback, controller](int /*n_press*/, double view_x, double view_y)
            {
                int event_x = 0;
                int event_y = 0;
                convert_widget_to_bin_window_coords(view, static_cast<int>(view_x), static_cast<int>(view_y), event_x, event_y);
