...
private:
    ...
    void update_filter_models(Torrent::ChangeFlags flags);
    void update_filter_models_idle(Torrent::ChangeFlags flags);
...
private:
    ...
    Glib::RefPtr<TorrentFilter> filter_;
    ...
    sigc::connection update_filter_models_idle_tag_;
};
...
bool FilterBar::Impl::tracker_filter_model_update()
{
    ...
    for (;;)
    {
        ...
        else if (insert_row)
        {
            ...
            add->set_value(tracker_filter_cols.type, static_cast<int>(TorrentFilter::TRACKER_HOST));
...
Glib::RefPtr<Gtk::TreeStore> FilterBar::Impl::tracker_filter_model_new()
{
    ...
    iter->set_value(tracker_filter_cols.type, static_cast<int>(TorrentFilter::TRACKER_ALL));
...
{
    cell_renderer.property_width() = (iter->get_value(tracker_filter_cols.type) == TorrentFilter::TRACKER_HOST) ? 20 : 0;
}
...
bool FilterBar::Impl::activity_filter_model_update()
{
    ...
    for (auto& row : activity_model_->children())
    {
        ...
        {
            auto const torrent = gtr_ptr_dynamic_cast<Torrent>(torrents_model->get_object(i));
            if (torrent != nullptr && TorrentFilter::test_activity(*torrent, static_cast<TorrentFilter::Activity>(type)))
            {
                ++hits;
            }
        }
...
Glib::RefPtr<Gtk::ListStore> FilterBar::Impl::activity_filter_model_new()
{
    ...
    static auto const types = std::array<FilterTypeInfo, 9>({ {
        { TorrentFilter::ACTIVITY_ALL, nullptr, N_("All"), {} },
        { -1, nullptr, nullptr, {} },
        { TorrentFilter::ACTIVITY_ACTIVE, nullptr, N_("Active"), "system-run" },
        { TorrentFilter::ACTIVITY_DOWNLOADING, "Verb", NC_("Verb", "Downloading"), "network-receive" },
        { TorrentFilter::ACTIVITY_SEEDING, "Verb", NC_("Verb", "Seeding"), "network-transmit" },
        { TorrentFilter::ACTIVITY_PAUSED, nullptr, N_("Paused"), "media-playback-pause" },
        { TorrentFilter::ACTIVITY_FINISHED, nullptr, N_("Finished"), "media-playback-stop" },
        { TorrentFilter::ACTIVITY_VERIFYING, "Verb", NC_("Verb", "Verifying"), "view-refresh" },
        { TorrentFilter::ACTIVITY_ERROR, nullptr, N_("Error"), "dialog-error" },
    } });
    ...
    Gtk::TreeModel::const_iterator const& iter)
{
    ...
    cell_renderer.property_width() = type == TorrentFilter::ACTIVITY_ALL ? 0 : 20;
    cell_renderer.property_ypad() = type == TorrentFilter::ACTIVITY_ALL ? 0 : 2;
}
...
void FilterBar::Impl::update_filter_activity()
{
    ...
    {
        filter_->set_activity(static_cast<TorrentFilter::Activity>(iter->get_value(activity_filter_cols.type)));
    }
    else
    {
        filter_->set_activity(TorrentFilter::ACTIVITY_ALL);
    }
}
...
void FilterBar::Impl::update_filter_tracker()
{
    ...
    {
        filter_->set_tracker(
            static_cast<TorrentFilter::Tracker>(iter->get_value(tracker_filter_cols.type)),
            iter->get_value(tracker_filter_cols.sitename));
    }
    else
    {
        filter_->set_tracker(TorrentFilter::TRACKER_ALL, {});
    }
}
...
void FilterBar::Impl::update_filter_models(Torrent::ChangeFlags flags)
{
    static auto const activity_flags = Torrent::make_change_flags({
        Torrent::ChangeFlag::ACTIVE_PEERS_DOWN,
        Torrent::ChangeFlag::ACTIVE_PEERS_UP,
        Torrent::ChangeFlag::ACTIVE,
        Torrent::ChangeFlag::ACTIVITY,
        Torrent::ChangeFlag::ERROR_CODE,
        Torrent::ChangeFlag::FINISHED,
    });
    static auto const tracker_flags = Torrent::make_change_flags({ Torrent::ChangeFlag::TRACKERS });

    if ((flags & activity_flags).any())
    {
        activity_filter_model_update();
    }

    if ((flags & tracker_flags).any())
    {
        tracker_filter_model_update();
    }

    filter_->update(flags);
}

void FilterBar::Impl::update_filter_models_idle(Torrent::ChangeFlags flags)
{
...
{
    update_filter_models_idle_tag_ = core_->signal_torrents_changed().connect(
        sigc::mem_fun(*this, &Impl::update_filter_models_idle));
...
FilterBar::Impl::~Impl()
{
    update_filter_models_idle_tag_.disconnect();
